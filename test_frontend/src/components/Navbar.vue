...
<nav>
    ...
    <v-app-bar flat app class="px-2" color="white">
        ...
        <v-app-bar-nav-icon class="grey--text hidden-md-and-up" @click="hamburger=!hamburger"></v-app-bar-nav-icon>

        <!-- logo -->
        <v-btn plain color="yellow darken-3" to="/">
        <v-img :src="require('../assets/BeeLogoSmall.png')" class="mr-2" max-height="50" max-width="50" contain/>
    ...
    <div class="hidden-sm-and-down">
        <span v-for="(link,i) in links" :key="i">
            <v-btn v-if="i<4" plain depressed color="grey darken-4" :to="link.route" 
            class="font-weight-bold hidden-sm-only">
            <v-icon small left>{{link.icon}}</v-icon>
            <span plain color="grey darken-4" class="text-body-2 font-weight-bold">{{ link.text }}</span>
        </v-btn>
        </span>
        <v-btn v-if="userLoggedIn" @click="onLogout" plain depressed color="grey darken-4">
            <v-icon small left>mdi-logout</v-icon>
                <span plain color="grey darken-4" class="text-body-2 font-weight-bold">Logout</span>
        </v-btn>
